...
export default async () => {
  ...
  return (
    <>
      ...
      {/* 新增公网安备部分 */}
      {web?.publicSecurityRecord && (
        <div className='group flex justify-center items-center space-x-2 pb-4 cursor-pointer'>
          {/* 公网安备图标 */}
          <Link href={`https://beian.mps.gov.cn/#/query/webSearch?code=${web?.publicSecurityRecord?.replace(/[^0-9]/g, '')}`} 
      target="_blank">
            <Image src={GH} alt="公网安备" className='w-5 h-5 mr-1' width={20} height={20}/>
          </Link>
          {/* 公网安备文本链接 */}
          <Link href={`https://beian.mps.gov.cn/#/query/webSearch?code=${web?.publicSecurityRecord?.replace(/[^0-9]/g, '')}`} target="_blank" className='group-hover:text-primary '>
            {web?.publicSecurityRecord}
          </Link>
        </div>
      )}
        ...
        <div className="py-4 border-t dark:border-black-a  ">
          <Tooltip content="一款免费、开源、年轻、高颜值的现代化博客管理系统">
            <div className="flex justify-center items-center space-x-3">
              <img src="https://bu.dusays.com/2025/05/28/683721bef3de6.png" width={30} height={30} alt="龙氏 ThriveX 博客管理系统" />
